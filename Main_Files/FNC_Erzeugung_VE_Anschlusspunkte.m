function [M_A] = FNC_Erzeugung_VE_Anschlusspunkte(D,Alpha,tau_M,N,tau_S,mode_S,tau_C)
%FNC_Erzeugung_VE
%   Eingabewerte:
...
n      = N(1);
nRE    = size(D,1);                    % Anzahl der Rohrelemente
...
%% Bestimmung von Krümmungsmittelpunkten, Carinalverrundungsradien und Konstruktionsradien in den Carinalverrundungsflächen
...
for w = 1 : nRE-2
    for j = 1 : n/2
        ...
        else % vermeidet numerische Rundungsungenauigkeit: cos(pi/2) != 0
            B(j,:,1,w) = [-sign(Alpha(w+1))*cos(Phi(w+1)),0,sin(Phi(w+1))]*R_K(j,1,w) + M_K(w+1,:);
            B(j,:,2,w) = [-sign(Alpha(w+2))*cos(Phi(w+2)),0,sin(Phi(w+2))]*R_K(j,2,w) + M_K(w+2,:);
        end
